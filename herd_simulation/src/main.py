...
def load_game(filename):
    ...
    for i in range(n_cars):
        ...
        while success == 0:
            ...
            finish = [random.uniform(info[4], info[5]) for i in range(2)]           #randomly generating x,y coordinates

            if check_path_length(start,finish) < 650 or check_overlap(finish,world) != 0:   #if path is too short or the the vehicles end up on top of each other
                success = 0
            else:
                success += 1
        ...
        car = Vehicle(fm, max_speed, direction, path, size,world)  # creating a car-object and adding it to the world
...
def check_overlap(finish,world):
    ...
    for i in range(len(vehicles)):
        ...
        if check_path_length(other_finish,finish) < 50:         #finishing locations are too close to each other
            overlap += 1
...
def check_input(list):
    ...
    try:
        ...
        info = [float(list[i]) for i in range(len(list))]  # list of parameters for setting up the car.
        ...
        if n_cars == 0:
            raise CarInfoFileError("{} is an invalid amount of cars! Should be more than 0".format(n_cars))

        if info[1] < 30 or info[1] > 50:
            raise CarInfoFileError("{} is an invalid car size! Should be within 30 and 50.".format(info[1]))

        if info[2] < 0.05 or info[2] > 0.2:
            raise CarInfoFileError("{}-{} invalid force/mass range! Min: 0.05, Max: 0.2".format(info[2],info[3]))
        ...
        if info[6] > info[7] or info[6] < 0.75 or info[7] > 2.5:
            raise CarInfoFileError(
                "{}-{} is an incorrect range for max speed!. Should be 0.75-2.50".format(info[10], info[11]))
        return info

    except ValueError:  # If any of info inputs are invalid (characters, etc.) a valueError is raised
